...
class ACO_Helper_Cart {
	...
	public function get_shipping() {
		...
		foreach ( $packages as $i => $package ) {
			...
			foreach ( $package['rates'] as $rate ) {
				...
				if ( ACO_WC()->checkout->is_integrated_shipping_enabled() || ACO_WC()->checkout->is_integrated_wc_shipping_enabled() ) {
					...
					// Avarda falls back on this item if the shipping broker fails, so it has to be the rate WooCommerce actually charges for.
					if ( $chosen_shipping === $rate_id ) {
						return $shipping_item;
					}

					if ( null === $default_shipping ) {
						$default_shipping = $shipping_item;
					}
		...
		return $formatted_shipping ?? $default_shipping;
	}
